...
describe("audience storage capacity", () => {
	it("preserves four distinct audience choices without using the overflow bin", () => {
		const totals = new Map<string, number>();
		for (const choice of ["Java", "Elixir", "Rust", "Zig"]) {
			const bin = boundedAudienceBin(new Set(totals.keys()), choice);
			totals.set(bin, (totals.get(bin) ?? 0) + 1);
		}
		...
		expect(totals.has(AUDIENCE_OTHER_BIN)).toBe(false);
		expect(validAudienceTotals(Object.fromEntries(totals))).toBe(true);
	});

	it("bounds 4,000 distinct maximum-length answers across 32 synthetic shards", () => {
		...
		expect(Object.values(totals).reduce((sum, total) => sum + total, 0)).toBe(4_000);
