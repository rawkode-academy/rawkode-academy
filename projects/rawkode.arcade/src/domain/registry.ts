...
function project(state: GameState, runtime: Runtime, reducerState: unknown): GameState {
	...
	if (runtime.key === "spinlock") { const letters = Array.isArray(view.letters) ? view.letters.filter((letter): letter is string => typeof letter === "string") : []; const solved = typeof view.solvedBy === "string"; const phrase = (runtime.content as typeof spinlockSeed).rounds[Number(view.roundIndex)]?.phrase ?? ""; const board = [...phrase].map((character) => /[a-z0-9]/i.test(character) ? solved || view.revealed === true || letters.includes(character.toUpperCase()) ? character.toUpperCase() : "▢" : character).join(""); next.spinlock = { board, letters, activeValue: Number(view.activeValue) || 0, turn: Number(view.turn) || 0, solved }; }
	else next.spinlock = undefined;
	if (runtime.key === "merge-conflict") { const round = (runtime.content as typeof mergeConflictSeed).rounds[Number(view.roundIndex)]; const revealed = new Set(Array.isArray(view.revealed) ? view.revealed.filter((answer): answer is string => typeof answer === "string") : []); next.gameBoard = { kind: "merge-conflict", total: round?.answers.length ?? 0, entries: (round?.answers ?? []).map((answer, index) => ({ rank: index + 1, ...(revealed.has(answer.answer) ? { label: answer.answer } : {}), revealed: revealed.has(answer.answer) })) }; }
	else if (runtime.key === "spinlock" && next.spinlock) next.gameBoard = { kind: "spinlock", board: next.spinlock.board, letters: next.spinlock.letters, activeValue: next.spinlock.activeValue, solved: Boolean(next.spinlock.solved) };
	else if (runtime.key === "principal-engineer") { const questions = (runtime.content as typeof principalEngineerSeed).questions; next.gameBoard = { kind: "principal-engineer", index: Math.max(0, Number(view.questionIndex) || 0), total: questions.length }; }
	else if (runtime.key === "race-condition") { const rawPositions = view.positions && typeof view.positions === "object" ? view.positions as Record<string, unknown> : {}; const teamPositions = Object.fromEntries(Object.entries(rawPositions).filter(([, position]) => Number.isSafeInteger(position) && (position as number) >= 0)) as Record<string, number>; next.gameBoard = { kind: "race-condition", teamPositions, playerPosition: Math.max(0, ...Object.values(teamPositions)), chaserPosition: Math.max(0, Number(view.chaserPosition) || 0), total: Math.max(0, Number((runtime.content as typeof raceConditionSeed).finish) || 0) }; }
	else if (runtime.key === "ten-nines") { const round = (runtime.content as typeof tenNinesSeed).rounds[Number(view.roundIndex)]; next.gameBoard = { kind: "ten-nines", found: Array.isArray(view.found) ? view.found.filter((answer): answer is string => typeof answer === "string") : [], total: round?.answers.length ?? 0 }; }
	else if (runtime.key === "null-pointer" && (view.phase === "revealed" || view.phase === "complete")) next.gameBoard = { kind: "null-pointer", distribution: Array.isArray(view.revealedAnswers) ? (view.revealedAnswers as Array<{ answer?: unknown; points?: unknown }>).filter((entry) => typeof entry.answer === "string" && Number.isSafeInteger(entry.points) && (entry.points as number) >= 0).map((entry) => ({ label: entry.answer as string, count: entry.points as number })) : [] };
	else next.gameBoard = undefined;
	...
	if (Array.isArray(view.revealed)) next.revealedAnswer = view.revealed.join(" · "); else if (Array.isArray(view.revealedAnswers)) next.revealedAnswer = (view.revealedAnswers as Array<{ answer?: string }>).map((entry) => entry.answer).filter(Boolean).join(" · "); else if (runtime.key === "spinlock" && view.revealed === true) next.revealedAnswer = (runtime.content as typeof spinlockSeed).rounds[Number(view.roundIndex)]?.phrase; else if (runtime.key === "spinlock" && typeof view.solvedBy === "string") next.revealedAnswer = "Solved"; else if (runtime.key === "principal-engineer" && view.revealed === true) { const question = (runtime.content as typeof principalEngineerSeed).questions[Number(view.questionIndex)]; next.revealedAnswer = question?.choices[question.correct]; } else if (runtime.key === "ten-nines" && view.revealed === true) next.revealedAnswer = (runtime.content as typeof tenNinesSeed).rounds[Number(view.roundIndex)]?.answers.map((answer) => answer.answer).join(" · "); else next.revealedAnswer = undefined;
	return next;
}
