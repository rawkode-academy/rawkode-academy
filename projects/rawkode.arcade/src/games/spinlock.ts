...
export const spinlock = (content: SpinlockContent): GameDefinition<SpinlockContent, SpinlockState, SpinlockCommand> => ({
	...
	handle: (state, command, actor) => {
		...
		if (command.type === "audience-guess" && actor.role === "audience" && state.phase === "open") return next(state, { audienceGuesses: { ...state.audienceGuesses, [actor.id]: command.letter } });
		if (command.type === "reveal" && isHost(actor) && state.phase === "open" && state.solvedBy) return next(state, { revealed: true });
		if (command.type === "next-round" && isHost(actor) && state.phase === "open" && state.solvedBy) return state.roundIndex + 1 >= content.rounds.length ? next(state, { phase: "complete" }) : next(state, { roundIndex: state.roundIndex + 1, letters: [], solvedBy: undefined, revealed: undefined, activeValue: 0, audienceGuesses: {} });
