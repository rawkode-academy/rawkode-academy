import {
	guestLayout,
	normalizeGuests,
	type ThumbnailRenderParams,
} from "./contracts";

export interface RenderThumbnailHtmlInput {
	params: ThumbnailRenderParams;
	backgroundImageDataUrl: string;
	technologyIconSvg: string;
}

const RAWKODE_WORDMARK_SVG = `<svg width="512" height="128" viewBox="0 0 512 128" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M93.3877 51.4242H58.2967V61.3828H93.3877C93.6991 61.3828 93.9325 61.6109 93.9325 61.9151V68.4526H58.2967V85.0248C58.2967 90.8023 63.1208 95.5156 69.0343 95.5156H104.125V61.9151C104.125 56.1376 99.3011 51.4242 93.3877 51.4242H93.3877ZM93.9325 85.5571H69.0343C68.7228 85.5571 68.4893 85.329 68.4893 85.0248V78.4873H93.9325V85.5571ZM397.751 61.9151V95.5156H387.636V61.9151C387.636 61.6109 387.402 61.3828 387.013 61.3828H370.051C369.74 61.3828 369.584 61.6109 369.584 61.9151V95.5156H359.314V61.9151C359.314 61.6109 359.08 61.3828 358.847 61.3828H341.807C341.496 61.3828 341.262 61.6109 341.262 61.9151V95.5156H331.069V51.4242H387.013C392.926 51.4242 397.751 56.1376 397.751 61.9151ZM310.144 51.4242H285.79C279.877 51.4242 275.053 56.1376 275.053 61.9151V85.0248C275.053 90.8023 279.877 95.5156 285.79 95.5156H320.881V85.5571H285.79C285.479 85.5571 285.245 85.329 285.245 85.0248V78.4873H320.881V61.9151C320.881 56.1376 316.057 51.4242 310.144 51.4242ZM310.688 68.4526H285.245V61.9151C285.245 61.6109 285.479 61.3828 285.79 61.3828H310.144C310.455 61.3828 310.689 61.6109 310.689 61.9151V68.4526H310.688ZM454 51.5766V102.509C454 108.287 449.254 113 443.34 113H415.719V102.965H443.34C443.574 102.965 443.807 102.813 443.807 102.509V95.5156H418.909C412.996 95.5156 408.172 90.8023 408.172 85.0248V51.5766H418.365V85.0248C418.365 85.329 418.598 85.5571 418.909 85.5571H443.34C443.574 85.5571 443.807 85.329 443.807 85.0248V51.5766H454ZM202.233 51.4242H167.142V61.3828H202.233C202.544 61.3828 202.778 61.6109 202.778 61.9151V68.4526H167.142V85.0248C167.142 90.8023 171.966 95.5156 177.879 95.5156H212.97V61.9151C212.97 56.1376 208.146 51.4242 202.233 51.4242H202.233ZM202.778 85.5571H177.879C177.568 85.5571 177.334 85.329 177.334 85.0248V78.4873H202.778V85.5571ZM256.538 51.4242H231.562C225.648 51.4242 220.824 56.1376 220.824 61.9151V85.0248C220.824 90.8023 225.648 95.5156 231.562 95.5156H266.73V36.981H256.538V51.4242ZM256.538 85.0248C256.538 85.329 256.304 85.5571 255.993 85.5571H231.562C231.25 85.5571 231.095 85.329 231.095 85.0248V61.9151C231.095 61.6109 231.25 61.3828 231.562 61.3828H255.993C256.304 61.3828 256.538 61.6109 256.538 61.9151V85.0248ZM122.951 85.5571H158.042V95.5156H122.951C117.037 95.5156 112.213 90.8023 112.213 85.0248V61.9151C112.213 56.1376 117.037 51.4242 122.951 51.4242H157.886V61.3828H122.951C122.639 61.3828 122.406 61.6109 122.406 61.9151V85.0248C122.406 85.329 122.639 85.5571 122.951 85.5571ZM237.123 38.5963V34.186H221.583C221.445 34.186 221.341 34.085 221.341 33.9504V31.055H237.123V23.7158C237.123 21.1571 234.987 19.0698 232.368 19.0698H221.583C218.964 19.0698 216.827 21.1571 216.827 23.7158V33.9504C216.827 36.5091 218.964 38.5964 221.583 38.5964H237.123L237.123 38.5963ZM221.341 23.7158C221.341 23.5811 221.445 23.4802 221.583 23.4802H232.368C232.506 23.4802 232.609 23.5811 232.609 23.7158V26.6111H221.341V23.7158H221.341ZM173.229 38.5963H184.015C186.634 38.5963 188.77 36.5091 188.77 33.9504V23.7158C188.77 21.1571 186.634 19.0698 184.015 19.0698H173.229C170.61 19.0698 168.474 21.1571 168.474 23.7158V33.9504C168.474 36.5091 170.61 38.5964 173.229 38.5964V38.5963ZM172.988 23.7158C172.988 23.5811 173.091 23.4802 173.229 23.4802H184.015C184.152 23.4802 184.256 23.5811 184.256 23.7158V33.9504C184.256 34.085 184.152 34.186 184.015 34.186H173.229C173.091 34.186 172.988 34.085 172.988 33.9504V23.7158V23.7158ZM160.79 19.0698H165.649V20.2144L157.654 28.8329L165.649 37.4516V38.5963H160.79L154.421 31.5527L150.108 35.9426V38.5963H145.594V14H150.108V30.1924L153.657 26.6111L160.79 19.0698L160.79 19.0698ZM100.847 19.0698H85.306V23.4802H100.847C100.985 23.4802 101.088 23.5811 101.088 23.7158V26.6111H85.306V33.9504C85.306 36.509 87.4425 38.5963 90.0612 38.5963H105.602V23.7158C105.602 21.1571 103.466 19.0698 100.847 19.0698L100.847 19.0698ZM101.088 34.186H90.0612C89.9235 34.186 89.8201 34.085 89.8201 33.9504V31.055H101.088V34.1861V34.186ZM197.268 38.5963H212.843V14H208.329V19.0698H197.268C194.649 19.0698 192.513 21.1571 192.513 23.7158V33.9504C192.513 36.5091 194.649 38.5964 197.268 38.5964V38.5963ZM197.061 23.7158C197.061 23.5811 197.13 23.4802 197.268 23.4802H208.088C208.226 23.4802 208.329 23.5811 208.329 23.7158V33.9504C208.329 34.085 208.226 34.186 208.088 34.186H197.268C197.13 34.186 197.061 34.085 197.061 33.9504V23.7158V23.7158ZM115.812 38.5963L107.955 19.0698H112.745L117.569 30.5838C119.258 26.7794 121.05 22.8404 122.704 19.0698H127.39L132.834 30.651C134.351 26.8467 135.901 22.8404 137.348 19.0698H142.138L134.626 38.5963H131.112L125.081 25.5338L119.292 38.5963H115.812H115.812ZM65.0315 27.5691C67.9777 24.0148 72.5215 18.4882 75.9637 14.2726H82.5662C79.1442 18.111 75.6285 22.0714 72.2198 26.1632L71.9526 26.4838L72.2278 26.798C72.6331 27.2605 80.173 35.7607 82.5739 38.4671H76.5869C75.8178 37.6005 74.9329 36.6141 74.0358 35.614C72.2854 33.6622 70.4753 31.6443 69.4067 30.386L69.021 29.932L68.6301 30.3819C67.4278 31.7657 66.2063 33.1543 65.0316 34.4845V27.5691H65.0315ZM62.8528 36.9899L58 42.5842V17.1245C58.001 15.5512 59.3069 14.2763 60.9172 14.2762L73.2043 14.2739C72.6212 14.9987 72.0306 15.7199 71.4543 16.4236C70.755 17.2777 70.0334 18.1587 69.3301 19.0401H62.8528V36.9899H62.8528Z" fill="currentColor"/></svg>`;

export function escapeHtml(value: string): string {
	return value
		.replaceAll("&", "&amp;")
		.replaceAll("<", "&lt;")
		.replaceAll(">", "&gt;")
		.replaceAll('"', "&quot;")
		.replaceAll("'", "&#39;");
}

function stripSvgPreamble(svg: string): string {
	let value = svg.trimStart();

	while (true) {
		const trimmed = value.trimStart();
		const lower = trimmed.toLowerCase();

		if (lower.startsWith("<?xml")) {
			const end = trimmed.indexOf("?>");
			if (end === -1) throw new Error("Technology icon XML declaration is not closed");
			value = trimmed.slice(end + 2);
			continue;
		}

		if (lower.startsWith("<!doctype")) {
			const end = trimmed.indexOf(">");
			if (end === -1) throw new Error("Technology icon doctype is not closed");
			value = trimmed.slice(end + 1);
			continue;
		}

		if (trimmed.startsWith("<!--")) {
			const end = trimmed.indexOf("-->");
			if (end === -1) throw new Error("Technology icon comment is not closed");
			value = trimmed.slice(end + 3);
			continue;
		}

		return trimmed.trim();
	}
}

export function technologyIconDataUrl(svg: string): string {
	const trimmed = stripSvgPreamble(svg);
	if (!/^<svg[\s>]/i.test(trimmed)) {
		throw new Error("Technology icon must be inline SVG");
	}

	const bytes = new TextEncoder().encode(trimmed);
	let binary = "";
	for (const byte of bytes) {
		binary += String.fromCharCode(byte);
	}

	return `data:image/svg+xml;base64,${btoa(binary)}`;
}

export function renderVideoThumbnailHtml({
	params,
	backgroundImageDataUrl,
	technologyIconSvg,
}: RenderThumbnailHtmlInput): string {
	const guests = normalizeGuests(params.guests);
	const layout = guestLayout(guests.length);
	const label = escapeHtml(params.show?.name ?? "Hands-on");
	const tagline = params.video.tagline?.trim()
		? escapeHtml(params.video.tagline.trim())
		: "";
	const technologyName = escapeHtml(params.technology.name);
	const technologyIconSrc = technologyIconDataUrl(technologyIconSvg);

	const guestMarkup = guests
		.map(
			(guest) =>
				`<div class="avatar"><img src="${escapeHtml(guest.avatarUrl)}" alt="${escapeHtml(guest.name)}" /></div>`,
		)
		.join("");

	const guestNames = guests.map((guest) => guest.name).join(", ");

	return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=1280,height=720,initial-scale=1" />
    <title>${label} Thumbnail</title>
    <style>
      :root { color-scheme: dark; --iroh:#7c7cff; --cyan:#65e8ff; --ink:#03050a; }
      * { box-sizing: border-box; }
      html, body {
        margin: 0;
        width: 1280px;
        height: 720px;
        overflow: hidden;
        background: #020308;
        font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
      }
      .thumbnail { position: relative; width: 1280px; height: 720px; isolation: isolate; background: #050711; }
      .bg {
        position: absolute;
        inset: 0;
        background-image: url("${backgroundImageDataUrl}");
        background-size: cover;
        background-position: center;
        transform: scale(1.08);
        filter: blur(2.4px) saturate(1.14) contrast(1.04);
      }
      .black-layer { position: absolute; inset: 0; background: rgba(0, 0, 0, .46); }
      .center-shield {
        position: absolute;
        left: 50%;
        top: 47%;
        transform: translate(-50%, -50%);
        width: 660px;
        height: 390px;
        border-radius: 999px;
        background: radial-gradient(ellipse at center, rgba(0, 0, 0, .7) 0 34%, rgba(0, 0, 0, .5) 50%, rgba(0, 0, 0, .18) 68%, transparent 82%);
        filter: blur(14px);
        pointer-events: none;
      }
      .vignette {
        position: absolute;
        inset: -1px;
        background:
          radial-gradient(circle at 50% 45%, rgba(124, 124, 255, .18) 0 16%, rgba(0, 0, 0, 0) 42%),
          radial-gradient(circle at 78% 55%, rgba(101, 232, 255, .14), transparent 31%),
          linear-gradient(180deg, rgba(0, 0, 0, .22), rgba(0, 0, 0, .66));
        mix-blend-mode: screen;
        opacity: .78;
        pointer-events: none;
      }
      .brand {
        position: absolute;
        top: 42px;
        left: 50px;
        width: 236px;
        color: #f7f8fb;
        opacity: .92;
        filter: drop-shadow(0 7px 16px rgba(0, 0, 0, .42));
      }
      .brand svg { display: block; width: 100%; height: auto; }
      .center {
        position: absolute;
        left: 50%;
        top: 47%;
        transform: translate(-50%, -50%);
        width: 350px;
        height: 350px;
        display: grid;
        place-items: center;
      }
      .glow {
        position: absolute;
        inset: 0;
        border-radius: 999px;
        background: radial-gradient(circle, rgba(124, 124, 255, .46), rgba(124, 124, 255, .24) 35%, rgba(101, 232, 255, .12) 52%, transparent 69%);
        filter: blur(12px);
      }
      .plate {
        position: absolute;
        inset: 37px;
        border-radius: 999px;
        background: radial-gradient(circle at 42% 30%, rgba(255, 255, 255, .14), rgba(124, 124, 255, .12) 36%, rgba(10, 12, 26, .54) 70%);
        border: 1px solid rgba(220, 225, 255, .2);
        box-shadow: 0 0 72px rgba(124, 124, 255, .55), 0 0 132px rgba(101, 232, 255, .18), inset 0 0 42px rgba(255, 255, 255, .07);
      }
      .icon-wrap {
        position: relative;
        z-index: 2;
        width: 190px;
        height: 190px;
        display: grid;
        place-items: center;
        filter: drop-shadow(0 0 18px rgba(124, 124, 255, .98)) drop-shadow(0 0 48px rgba(101, 232, 255, .46));
      }
      .technology-svg { width: 100%; height: 100%; }
      .kicker {
        position: absolute;
        left: 64px;
        bottom: 64px;
        display: flex;
        flex-direction: column;
        gap: 10px;
        max-width: 560px;
      }
      .label {
        width: max-content;
        padding: 8px 13px 7px;
        border: 1px solid rgba(255, 255, 255, .2);
        background: rgba(0, 0, 0, .42);
        color: #dfe5ff;
        font-size: 21px;
        line-height: 1;
        letter-spacing: .08em;
        text-transform: uppercase;
        font-weight: 800;
        box-shadow: 0 12px 24px rgba(0, 0, 0, .24);
      }
      .tagline {
        color: #fff;
        font-size: 39px;
        line-height: 1.04;
        font-weight: 860;
        letter-spacing: 0;
        text-shadow: 0 5px 28px rgba(0, 0, 0, .72);
        text-wrap: balance;
      }
      .guests {
        --guest-size: ${layout.size}px;
        --guest-columns: ${layout.columns};
        position: absolute;
        right: 58px;
        bottom: 52px;
        display: ${layout.count > 0 ? "grid" : "none"};
        gap: 12px;
        grid-template-columns: repeat(var(--guest-columns), var(--guest-size));
        width: max-content;
      }
      .avatar {
        width: var(--guest-size);
        height: var(--guest-size);
        border-radius: 999px;
        padding: 5px;
        background: linear-gradient(135deg, rgba(255, 255, 255, .92), rgba(124, 124, 255, .92) 54%, rgba(101, 232, 255, .86));
        box-shadow: 0 0 42px rgba(124, 124, 255, .55), 0 14px 40px rgba(0, 0, 0, .54);
      }
      .avatar img {
        display: block;
        width: 100%;
        height: 100%;
        border-radius: inherit;
        object-fit: cover;
        border: 3px solid rgba(4, 6, 13, .85);
      }
      .guest-caption {
        grid-column: 1 / -1;
        justify-self: end;
        color: #eef2ff;
        font-size: 18px;
        font-weight: 800;
        text-shadow: 0 5px 20px rgba(0, 0, 0, .82);
        opacity: .92;
      }
      .scanline {
        position: absolute;
        inset: 0;
        background: repeating-linear-gradient(180deg, rgba(255, 255, 255, .024) 0 1px, transparent 1px 5px);
        opacity: .22;
        pointer-events: none;
      }
    </style>
  </head>
  <body>
    <main class="thumbnail" role="img" aria-label="${label} ${technologyName} thumbnail">
      <div class="bg"></div>
      <div class="black-layer"></div>
      <div class="center-shield"></div>
      <div class="vignette"></div>
      <div class="brand" aria-label="Rawkode Academy">${RAWKODE_WORDMARK_SVG}</div>
      <section class="center" aria-label="${technologyName} technology icon">
        <div class="glow"></div>
        <div class="plate"></div>
        <div class="icon-wrap"><img class="technology-svg" src="${technologyIconSrc}" alt="" /></div>
      </section>
      <section class="kicker">
        <div class="label">${label}</div>
        ${tagline ? `<div class="tagline">${tagline}</div>` : ""}
      </section>
      <section class="guests" data-count="${layout.count}" aria-label="Guests">
        ${guestMarkup}
        ${guestNames ? `<div class="guest-caption">${escapeHtml(guestNames)}</div>` : ""}
      </section>
      <div class="scanline"></div>
    </main>
  </body>
</html>`;
}
