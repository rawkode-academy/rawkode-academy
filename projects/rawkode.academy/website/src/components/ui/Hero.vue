	...
	<section :class="sectionClasses">
		...
		<div v-if="background !== 'none'" :class="css({ position: 'absolute', inset: '0', pointerEvents: 'none' })">
			...
			<div v-if="background === 'blobs'" :class="css({ position: 'absolute', inset: '0' })">
				<div :class="css({ position: 'absolute', top: '0', left: '25%', w: '96', h: '96', borderRadius: 'full', opacity: 0.2, filter: 'blur(64px)', bg: 'rgba(var(--brand-secondary), 0.2)' })"></div>
				<div :class="css({ position: 'absolute', bottom: '0', right: '25%', w: '96', h: '96', borderRadius: 'full', opacity: 0.2, filter: 'blur(64px)', bg: 'rgba(var(--brand-primary), 0.2)' })"></div>
			</div>
...
const sectionClasses = computed(() => {
	const backgroundStyles: Record<string, string> = {
		...
		gradient: css({
			backgroundImage: {
				base: "linear-gradient(to bottom right, rgba(var(--brand-primary), 0.1), white, rgba(var(--brand-secondary), 0.1))",
				_dark: "linear-gradient(to bottom right, rgba(var(--brand-primary), 0.2), var(--colors-gray-900), rgba(var(--brand-secondary), 0.2))",
			},
		}),
		"gradient-hero": css({
			backgroundImage: "linear-gradient(135deg, rgba(var(--brand-primary), 0.15) 0%, rgba(var(--brand-secondary), 0.1) 50%, rgba(var(--brand-primary), 0.05) 100%)",
		}),
		"gradient-hero-alt": css({
			backgroundImage: "linear-gradient(225deg, rgba(var(--brand-secondary), 0.15) 0%, rgba(var(--brand-primary), 0.1) 50%, rgba(var(--brand-secondary), 0.05) 100%)",
		}),
		blobs: css({
			backgroundImage: {
...
const layoutClasses = computed(() => {
	const layoutMap: Record<string, string> = {
		...
		split: css({ display: "grid", gridTemplateColumns: { base: "1", lg: "2" }, gap: "12", alignItems: "center" }),
