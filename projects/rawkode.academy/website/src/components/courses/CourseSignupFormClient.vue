<template>
  <div :class="css({ mt: '16', maxW: '2xl', mx: 'auto' })">
    ...
    <div :class="css({ bg: { base: 'white', _dark: 'gray.800' }, p: '8', borderRadius: 'xl', shadow: 'lg', borderWidth: '1px', borderColor: { base: 'gray.200/50', _dark: 'gray.700/50' } })">
      <!-- Checking Subscription -->
      <div v-if="checkingSubscription" :class="css({ textAlign: 'center', py: '8' })">
        <div :class="css({ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', w: '16', h: '16', borderRadius: 'full', bg: { base: 'gray.100', _dark: 'gray.700' }, mb: '6' })">
        ...
        <p :class="css({ fontSize: 'lg', color: 'fg.muted' })">Checking subscription status...</p>
      </div>

      <!-- Already Subscribed -->
      <div v-else-if="isAlreadySubscribed" :class="css({ textAlign: 'center', py: '8' })">
        <div :class="css({ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', w: '16', h: '16', borderRadius: 'full', bg: 'teal.500/10', mb: '6' })">
          <svg xmlns="http://www.w3.org/2000/svg" :class="css({ h: '8', w: '8', color: 'teal.500' })" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </div>
        <h3 :class="css({ fontSize: '2xl', fontWeight: 'bold', color: 'fg.default', mb: '3' })">You're Already Subscribed!</h3>
        <p :class="css({ fontSize: 'lg', color: 'fg.muted' })">
      ...
      <div v-else-if="submitted" :class="css({ textAlign: 'center', py: '8' })">
        <div :class="css({ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', w: '16', h: '16', borderRadius: 'full', bg: { base: 'green.100', _dark: 'green.900/30' }, mb: '6' })">
        ...
        <h3 :class="css({ fontSize: '2xl', fontWeight: 'bold', color: 'fg.default', mb: '3' })">Thank You!</h3>
        <p :class="css({ fontSize: 'lg', color: 'fg.muted', mb: '6' })">
      ...
      <!-- Form -->
      <template v-else>
        <p :class="css({ color: 'fg.muted', mb: '6', textAlign: 'center' })">
          Sign up to receive notifications when new content is available for this course.
        </p>

        <div v-if="error" :class="css({ mb: '6', p: '4', bg: { base: 'red.100', _dark: 'red.900/30' }, borderWidth: '1px', borderColor: { base: 'red.200', _dark: 'red.800' }, borderRadius: 'lg', color: { base: 'red.800', _dark: 'red.300' } })">
        ...
        <form @submit.prevent="submitForm" :class="css({ display: 'flex', flexDirection: 'column', gap: '4' })">
          <div v-if="!userEmail">
            <label for="email" :class="css({ display: 'block', fontSize: 'sm', fontWeight: 'medium', color: 'fg.muted', mb: '2' })">
              Email address
            </label>
            <input
              v-model="email"
              ...
              :class="css({ w: 'full', px: '4', py: '2', borderWidth: '1px', borderColor: { base: 'gray.300', _dark: 'gray.600' }, borderRadius: 'lg', bg: { base: 'white', _dark: 'gray.700' }, color: 'fg.default', _focus: { ringWidth: '2px', ringColor: 'teal.500', borderColor: 'teal.500' } })"
            />
          </div>

          <div v-if="disclaimer" :class="css({ fontSize: 'sm', color: 'fg.subtle', bg: { base: 'gray.50', _dark: 'gray.900' }, p: '3', borderRadius: 'lg' })">
            <p>{{ disclaimer }}</p>
          </div>

          <div v-if="allowSponsorContact && sponsor" :class="css({ display: 'flex', alignItems: 'flex-start' })">
            <input
              ...
              value="true"
              :class="css({ mt: '1', h: '4', w: '4', borderRadius: 'sm', borderColor: 'gray.300' })"
            />
            <label for="sponsor-contact" :class="css({ ml: '2', fontSize: 'sm', color: 'fg.muted' })">
          ...
          <button
            type="submit"
            :disabled="submitting"
            :class="css({ w: 'full', bg: 'teal.500', color: 'white', fontWeight: 'medium', py: '3', px: '4', borderRadius: 'lg', transition: 'colors', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', _hover: { bg: 'teal.600' }, _disabled: { opacity: '0.5', cursor: 'not-allowed' } })"
          >
