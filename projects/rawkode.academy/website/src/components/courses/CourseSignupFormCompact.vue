<template>
  <div>
    ...
    <!-- Form -->
    <form v-else @submit.prevent="submitForm" :class="css({ display: 'flex', flexDir: 'column', gap: '4' })">
      ...
      <div v-if="!userEmail">
        <input
          ...
          required
          :class="css({ w: 'full', px: '4', py: '3', bg: 'gray.900/50', borderWidth: '1px', borderColor: 'gray.600', rounded: 'lg', color: 'white', _placeholder: { color: 'gray.500' }, _focus: { outline: 'none', ringWidth: '2px', ringColor: 'primary', borderColor: 'transparent' } })"
          :disabled="loading"
        />
      </div>
      ...
      <div v-if="signupConfig.sponsor && signupConfig.allowSponsorContact" :class="css({ display: 'flex', alignItems: 'start', gap: '3' })">
        <input
          ...
          type="checkbox"
          id="sponsor-consent"
          :class="css({ mt: '1', w: '4', h: '4', color: 'primary', bg: 'gray.900', borderColor: 'gray.600', rounded: 'sm', _focus: { ringColor: 'primary', ringWidth: '2px' } })"
          :disabled="loading"
        />
      ...
      <button
        type="submit"
        :disabled="loading"
        :class="css({ w: 'full', py: '3', px: '4', bg: 'primary', color: 'white', fontWeight: 'semibold', rounded: 'lg', transition: 'colors', transitionDuration: '200ms', display: 'flex', alignItems: 'center', justifyContent: 'center', _hover: { bg: 'primary/90' }, _disabled: { bg: 'gray.700', cursor: 'not-allowed' } })"
      >
