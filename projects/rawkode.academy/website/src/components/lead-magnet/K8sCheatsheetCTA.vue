...
<template>
	<section :class="cx('glass-card-shimmer', css({ pos: 'relative', overflow: 'hidden', p: { base: '4', sm: '6', md: '8', lg: '12' }, rounded: { base: 'xl', sm: '2xl' } })) + ' bg-gradient-primary'">
		<!-- Dark overlay for text contrast -->
		<div :class="css({ pos: 'absolute', inset: '0', bg: 'black/30' })"></div>

		<!-- Background decoration using brand colors -->
		<div :class="css({ pos: 'absolute', inset: '0', opacity: '0.2' })">
			<div :class="css({ pos: 'absolute', top: '0', left: '0', w: '72', h: '72', bg: 'brand-secondary', rounded: 'full', filter: 'blur(48px)', transform: 'translate(-50%, -50%)' })"></div>
			<div :class="css({ pos: 'absolute', bottom: '0', right: '0', w: '96', h: '96', bg: 'brand-primary', rounded: 'full', filter: 'blur(48px)', transform: 'translate(33%, 33%)' })"></div>
		</div>

		<!-- Kubernetes helm icon decoration with slow rotation -->
		<div :class="css({ pos: 'absolute', top: { base: '2', sm: '4', md: '8' }, right: { base: '2', sm: '4', md: '8' }, opacity: '0.2', animation: 'spin 20s linear infinite' })">
		...
		<div :class="css({ pos: 'relative', zIndex: '10', display: 'flex', flexDir: { base: 'column', lg: 'row' }, alignItems: { lg: 'start' }, justifyContent: { lg: 'space-between' }, gap: { base: '6', sm: '8' } })">
			<!-- Content -->
			<div :class="css({ flex: '1', maxW: '2xl' })">
				<div :class="css({ display: 'inline-flex', alignItems: 'center', gap: '2', px: { base: '2.5', sm: '3' }, py: { base: '1', sm: '1.5' }, rounded: 'full', bg: 'black/30', color: 'white', fontSize: { base: 'xs', sm: 'sm' }, fontWeight: 'medium', mb: { base: '3', sm: '4' } })">
					<span :class="css({ pos: 'relative', display: 'flex', h: '2', w: '2' })">
						<span :class="css({ animation: 'ping', pos: 'absolute', display: 'inline-flex', h: 'full', w: 'full', rounded: 'full', bg: 'emerald.400', opacity: '0.75' })"></span>
						<span :class="css({ pos: 'relative', display: 'inline-flex', rounded: 'full', h: '2', w: '2', bg: 'emerald.400' })"></span>
					</span>
					Release: December 17, 2025
				</div>

				<h2 :class="css({ fontSize: { base: 'xl', sm: '2xl', md: '3xl', lg: '4xl' }, fontWeight: 'bold', color: 'white', mb: { base: '3', sm: '4' }, fontFamily: 'var(--font-display)', pr: { base: '12', sm: '0' } })">
				...
				<div :class="css({ display: 'grid', gridTemplateColumns: { base: '1', sm: '2' }, gap: '2', mb: { base: '5', sm: '6', lg: '0' } })">
					<button
						v-for="feature in features"
						:key="feature.id"
						@click="toggleFeature(feature.id)"
						:aria-expanded="isExpanded(feature.id)"
						:class="css({ textAlign: 'left', rounded: 'lg', p: { base: '2.5', sm: '3' }, transition: 'all', transitionDuration: '200ms', bg: 'black/20', cursor: 'pointer', _hover: { bg: 'black/30' }, _focus: { outline: 'none', ringWidth: '2px', ringColor: 'white/30' } }) + ' group'"
					>
						<div :class="css({ display: 'flex', alignItems: 'start', gap: { base: '2', sm: '3' } })">
							<!-- Category pill -->
							<span
								:class="[
									css({ display: 'inline-flex', alignItems: 'center', px: { base: '1.5', sm: '2' }, py: '0.5', rounded: 'sm', fontSize: { base: '2xs', sm: 'xs' }, fontWeight: 'semibold', color: 'white', flexShrink: '0', mt: '0.5' }),
			...
			<!-- Form Card -->
			<div :class="css({ w: { base: 'full', lg: 'auto' }, minW: { lg: '280px' }, maxW: { lg: '380px' } })">
				<Card
					variant="glass"
					padding="md"
					rounded="xl"
					shadow="elevated"
					:hover="false"
					:class="css({ ringWidth: '1px', ringColor: 'white/10' })"
				>
					<!-- Success State -->
					<Transition
					...
					>
						...
						>
							<div :class="css({ w: { base: '12', sm: '16' }, h: { base: '12', sm: '16' }, mx: 'auto', mb: { base: '3', sm: '4' }, rounded: 'full', bg: 'green.500/20', display: 'flex', alignItems: 'center', justifyContent: 'center' })">
							...
							<h3 :class="css({ fontSize: { base: 'lg', sm: 'xl' }, fontWeight: 'semibold', mb: '2', fontFamily: 'var(--font-display)' }) + ' text-primary-content'">You're on the list!</h3>
							<p :class="css({ fontSize: { base: 'xs', sm: 'sm' }, mb: { base: '4', sm: '6' } }) + ' text-secondary-content'">
								Your cheat sheet is ready. Click below to view it now.
							</p>
							<a
								href="/resources/kubernetes/1.35-cheatsheet"
								@click="trackViewAssetClick"
								:class="css({ display: 'flex', alignItems: 'center', justifyContent: 'center', w: 'full', py: { base: '2.5', sm: '3' }, px: '4', fontSize: { base: 'sm', sm: 'base' }, fontWeight: 'semibold', shadow: 'lg', _hover: { shadow: 'xl' } }) + ' btn-solid'"
							>
					...
					<div v-if="!showSuccessState">
						<h3 :class="css({ fontSize: { base: 'base', sm: 'lg' }, fontWeight: 'semibold', mb: { base: '1.5', sm: '2' }, fontFamily: 'var(--font-display)' }) + ' text-primary-content'">
							Get the free cheat sheet
						</h3>
						<p :class="css({ fontSize: { base: 'xs', sm: 'sm' }, mb: { base: '3', sm: '4' } }) + ' text-secondary-content'">
						...
						>
							<div
								v-if="error"
								:class="css({ mb: { base: '3', sm: '4' }, p: { base: '2.5', sm: '3' }, rounded: 'lg', bg: { base: 'red.50', _dark: 'red.900/20' }, borderWidth: '1px', borderColor: { base: 'red.200', _dark: 'red.800/50' } })"
							>
						...
						<template v-if="isSignedIn">
							<button
								@click="subscribeAsLearner"
								:disabled="isLoading"
								:class="css({ w: 'full', py: { base: '2.5', sm: '3' }, px: '4', fontSize: { base: 'sm', sm: 'base' }, fontWeight: 'semibold', shadow: 'lg', minH: { base: '44px', sm: '48px' }, _hover: { shadow: 'xl' }, _disabled: { opacity: '0.7', cursor: 'wait' } }) + ' btn-solid'"
							>
								<span :class="css({ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '2' })">
									<svg v-if="isLoading" :class="css({ w: { base: '4', sm: '5' }, h: { base: '4', sm: '5' }, animation: 'spin' })" fill="none" viewBox="0 0 24 24">
										...
										<path :class="css({ opacity: '0.75' })" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
									</svg>
						...
						<template v-else>
							<div :class="css({ display: 'flex', flexDir: 'column', gap: { base: '3', sm: '4' } })">
								<p :class="css({ fontSize: { base: 'xs', sm: 'sm' } }) + ' text-secondary-content'">
									Sign in with GitHub to get instant access to the cheat sheet.
								</p>
								<a
									:href="signInUrl"
									@click="trackSignInClick"
									:class="css({ display: 'flex', alignItems: 'center', justifyContent: 'center', w: 'full', py: { base: '2.5', sm: '3' }, px: '4', fontSize: { base: 'sm', sm: 'base' }, fontWeight: 'semibold', shadow: 'lg', minH: { base: '44px', sm: '48px' }, _hover: { shadow: 'xl' } }) + ' btn-solid'"
								>
						...
						<p :class="css({ mt: { base: '3', sm: '4' }, fontSize: { base: '2xs', sm: 'xs' }, textAlign: 'center' }) + ' text-muted'">
