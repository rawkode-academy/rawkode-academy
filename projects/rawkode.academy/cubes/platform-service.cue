...
#PlatformService: {
	...
	_ignorePatterns: [
		...
		".wrangler/",
		"",
		"# Managed files (regenerated by cuenv sync cubes)",
		"package.json",
		"tsconfig.json",
		"biome.json",
		"README.md",
		if includeDataModel {"drizzle.config.ts"},
		if includeReadModel {"read-model/wrangler.jsonc"},
		if includeReadModel {"read-model/main.ts"},
		if includeReadModel {"read-model/publish.ts"},
		if includeReadModel {"read-model/schema.gql"},
		if includeWriteModel {"write-model/wrangler.jsonc"},
		if includeHttp {"http/wrangler.jsonc"},
	]
