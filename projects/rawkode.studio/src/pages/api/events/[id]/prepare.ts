...
import { prepareLiveInput } from "@/core/live-actions";
...
export const POST: APIRoute = async (context) => {
	...
	try {
		...
		const updated = await prepareLiveInput(event, streamClientFromRequest());
		await store.put(updated);
		return jsonResponse(updated);
	} catch (error) {
		return errorResponse(error instanceof Error ? error.message : "Unable to prepare live input");
	}
};
