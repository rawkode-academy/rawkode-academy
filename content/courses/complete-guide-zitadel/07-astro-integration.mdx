...
Install Arctic for OAuth helpers, the JWT decoder used below, and the ID-token claim types:

```bash
bun add arctic @oslojs/jwt
bun add -d oidc-client-ts
```
...
export class Zitadel {
  ...
  constructor() {
    ...
    this.redirectURI = 'http://localhost:4321/api/auth/callback';
  }
