...
- **Auth:** GKE AutoPilot
    * We use GKE AutoPilot (with Spot) for consistently running services, such as [Zitadel](https://zitadel.com).
...
<Diagram id="academy-architecture" title="Academy read and write services" description="The website queries the Cosmo GraphQL gateway, which routes to five read services and their databases. Each database also connects to a corresponding write service behind the Restate RPC gateway.">
...
Follow our [BlueSky](https://bsky.app/profile/rawkode.academy) account to keep up to date with the platform build out and our new articles.
