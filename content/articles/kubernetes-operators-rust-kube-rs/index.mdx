...
Before we dive into code, let's address the obvious question: why would you choose Rust over Go for something so deeply embedded in the Go ecosystem?
...
Go's type system is intentionally simple. Rust's is intentionally expressive. When you're defining Custom Resource Definitions (CRDs), Rust's type system—with enums, pattern matching, and trait-based generics—lets you model your domain with precision that Go's structs and interfaces simply can't match.
...
In Go, you'd likely represent this as a string phase field with a collection of optional fields—losing the compiler's ability to enforce that you handle every state transition correctly. In Rust, a `match` on `DatabasePhase` forces you to handle every variant, making it impossible to forget an edge case.
...
That `#[derive(CustomResource)]` macro generates a complete `WebApp` struct with metadata, spec, and status—plus methods to generate the CRD YAML. Run `WebApp::crd()` and you get a fully-typed `CustomResourceDefinition` ready to apply to your cluster.
...
fn error_policy(
    ...
    // Exponential backoff on errors, starting at 10 seconds
...
async fn reconcile_deployment(
    ...
    let deployment = Deployment {
        metadata: kube::api::ObjectMeta {
            ...
            owner_references: Some(vec![webapp.controller_owner_ref(&())]),
            ..Default::default()
        },
...
Notice the `owner_references` line. This sets up Kubernetes garbage collection so that when a `WebApp` is deleted, its Deployment is automatically cleaned up. kube-rs provides `controller_owner_ref` as a convenience method—no manual UID wrangling required.
...
apiserver -> deployment: manages
apiserver -> service: manages
apiserver -> ingress: manages (if hostname set)
```
...
For integration tests, kube-rs works seamlessly with tools like `k3d` or `kind`. You can spin up an ephemeral cluster in CI:
...
The ecosystem around kube-rs is growing rapidly. Projects like **Krator** (Krustlet's actor-based operator framework) and **kopium** (which generates Rust types from existing CRDs) are filling gaps and reducing boilerplate. The community is smaller than Go's operator ecosystem, but it's active and welcoming.
...
- [ ] **Leader election**: Use kube-rs's `LeaderElection` to prevent split-brain in HA deployments
