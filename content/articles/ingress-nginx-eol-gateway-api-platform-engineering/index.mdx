...
That said, not everyone is in a position to replace their CNI at the same time as their ingress controller. I'd actually tell you not to (the migration checklist below covers why). Here's where the other options fit:
...
1. **Choose your implementation.** If you're on Cilium (and you should be), use Cilium Gateway API: it's a Helm flag away. If you're on GKE or AKS with first-party support, use that. Otherwise, Envoy Gateway. If you're not on Cilium yet, don't try to migrate your CNI and your gateway controller at the same time. Get the gateway migration done. Plan the move to Cilium as a separate project once things have settled.
...
Scramble through a copy-paste migration and you'll land in the same place with different YAML. Take the time to do this properly and you'll come out with a networking model that actually fits how your organization works.
