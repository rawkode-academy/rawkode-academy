...
Every Linux container you've ever run is, underneath, just a folder.
...
Do this on a disposable Linux VM or lab host you don't mind rebooting, not a production node or your laptop. If anything goes wrong (the cgroup isn't set up correctly, your shell isn't actually inside it, the limit isn't applied), the fork bomb at the end of this section will pin every CPU and exhaust the kernel's process table within seconds. Recovery means a hard reset. Read the whole section before running anything, and verify `cat /proc/self/cgroup` shows `0::/fork-demo` before you launch the bomb.
</Aside>

The memory demo killed a process. The `pids` controller does the opposite: it refuses to *create* the process. Same three primitives, same shape of interface, a different shape of "no."
...
That's how many `fork()` calls the kernel refused since the cgroup was created. Point it at an actual fork bomb from inside the same cgroup:

```bash
:(){ :|:& };:
```

The counter rockets into the millions in seconds. The rest of the system never notices. The wall holds.

Cleanup once the loop has exited:

```bash
kill $(jobs -p)                                   # reap the sleeps
echo $$ > /sys/fs/cgroup/user.slice/cgroup.procs  # leave the cgroup
rmdir /sys/fs/cgroup/fork-demo
```

`memory.max` killed. `pids.max` blocked. Same three primitives, two different verbs the kernel uses to enforce them.
...
Twelve lines. The same container ID, repeated across many hierarchies. The exact list varied by distro and runtime because related controllers could be co-mounted, but the important part is the same: one process had multiple cgroup memberships.
...
(There were also smaller indignities. The `cpuset` controller's `clone_children` quirk, where child cgroups didn't inherit the parent's CPU and memory node assignments unless you set a special flag, broke expectations everywhere else.)
...
`memory.low` and `memory.min` are entirely new concepts: memory *protection*. `memory.min` is a hard reclaim barrier within its **effective** range. The kernel propagates and caps `memory.min` against ancestor cgroups, so a child cannot protect more memory than its parents already protect. `memory.low` is best-effort protection on the same model. Together they let memory-sensitive workloads coexist with bursty ones on the same node without one starving the other.
...
Combined with `memory.events` (which counts OOM kills, reclaim activity, and high-watermark hits), you get much better visibility into the kernel's reclaim, throttling, and OOM behaviour for a specific workload, without having to attribute system-wide signals back to a guilty cgroup yourself.
...
These are systemd's words for cgroups. On most modern Kubernetes Linux nodes, systemd is the host's cgroup manager. It creates, names, and manages cgroup directories on the kernel's behalf. A `.slice` is systemd's name for a cgroup used to *organise* other cgroups. A `.scope` is systemd's name for a cgroup whose processes were started outside the unit itself. That's why cgroup paths on nodes using the systemd cgroup driver look like systemd named them. Because systemd did.
...
Kubernetes Quality of Service classes shape the cgroup layout and the kernel OOM score adjustment. Node-pressure eviction is a little more nuanced: the kubelet ranks pods by whether usage exceeds requests, Pod priority, and usage relative to requests. QoS is still the useful mental model: BestEffort tends to go first, Guaranteed tends to go last.

**Guaranteed** pods have CPU and memory requests equal to limits for every container, or the pod-level equivalent on Kubernetes versions where pod-level resources are enabled. They get their own slice directly under `kubepods.slice`. The kubelet sets their OOM score adjustment to -997, making them the last to be killed by the kernel OOM killer.
...
For kubeadm clusters, Kubernetes 1.22 made `systemd` the default when `cgroupDriver` is unset. The kubelet's own default is still `cgroupfs`, but Kubernetes recommends `systemd` when systemd is the init system, and Kubernetes requires the kubelet and runtime to use the systemd cgroup driver for cgroupsv2 support. If you're still on cgroupfs with systemd, you're running against the current.
